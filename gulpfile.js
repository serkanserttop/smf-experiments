...
gulp.task('build', function() {
	return gulp.src(all_files_in_order, {
			base: '.'
		})
		//.pipe(uglify())
...
gulp.task('es6', function() {
	return gulp.src('experiment/es6/es6.js', {
			base: '.'
		})
		.pipe(to5())
		.pipe(gulp.dest('smf'));
...
gulp.task('replace', function() {
	return gulp.src('Scripts/Pages.*.js', {
			base: './'
		})
...
gulp.task('replace-v2', function() {
	return gulp.src('Scripts/Pages.*.js', {
		...
		.pipe(replace(/App\.router\.define\(\'(.*)\'.*/, function(match, replacement) {
			replacement = replacement.replace(/\//g, '.')
				.replace('pages.', '', 'i');
			return 'App.helpers.definePage(\'' + replacement + '\', function(page, pageName){'
		}))
		.pipe(replace(/\'pages\/.*\'/, function(match) {
			match = match.replace(/\//g, '.')
				.replace('pages.', '', 'i');
...
gulp.task('create-app-main-js', function(){
  var p = 'Scripts', target_file = 'Scripts/App.pages.js';
  ...
  fs.readdir(p, function(err, files) {
    ...
    files.map(function(file) {
      ...
      .filter(function(file) {
        return (fs.statSync(file).isFile()) && (file.indexOf('Pages.') > -1);
      })
      .forEach(function(file) {
        fs.appendFile('Scripts/App.pages.js', 'load(\'' + file.replace('Scripts/', '') + '\');\n', function (err) {
          if (err) { throw err; }
        });
      });
  });
